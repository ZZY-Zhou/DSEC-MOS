...
from emoformer.models.utils import get_clones, get_activation_fn, expand
from emoformer.models import criterions
from emoformer.models.swin_transformer_3d import SwinTransformer3D
from emoformer.utils.misc import (NestedTensor, nested_tensor_from_tensor_list)
from emoformer.models.label_propagation import LabelPropagator
from emoformer.models.position_encoding import build_position_encoding
BN_MOMENTUM = 0.1
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

from emoformer.models.prior_gen import PriorGen
...
class VOS_Swin(nn.Module):
    ...
    def forward_inference(self, samples: NestedTensor):
        ...
        mean_outs = None
        with torch.no_grad():
            all_outs.append(self._forward_one_samples(samples))
...
class VOS_SwinLPROP(VOS_Swin):
    def __init__(self, args, backbone, backbone_dims, hidden_dim, transformer, num_frames,
        ...
        logger.debug('Initializing Swin-LPROP')
        # import ipdb;ipdb.set_trace()
        ...
        self.prior_gen0 = PriorGen(256)
        self.prior_gen1 = PriorGen(512)
        self.prior_gen2 = PriorGen(1024)
        self.prior_gen3 = PriorGen(1024)
    ...
    def _forward_one_samples(self, samples: NestedTensor):
        ...
        if self.backbone_name == 'resnet101' or self.backbone_name == 'resnet50':
            ...
            for i in range(len(features)):
                ...
                if i == 0:
                    x, pg = self.prior_gen0(x)
                elif i == 1:
                    x, pg = self.prior_gen1(x)
                elif i == 2:
                    x, pg = self.prior_gen2(x)
                else:
                    x, pg = self.prior_gen3(x)
        ...
        pred_priors[0] = pred_priors[0].permute(1, 0, 2, 3)
        pred_priors[1] = pred_priors[1].permute(1, 0, 2, 3)
        pred_priors[2] = pred_priors[2].permute(1, 0, 2, 3)
        pred_priors[3] = pred_priors[3].permute(1, 0, 2, 3)
        
        out["pred_priors_0"] = pred_priors[0]
        out["pred_priors_1"] = pred_priors[1]
        out["pred_priors_2"] = pred_priors[2]
        out["pred_priors_3"] = pred_priors[3]
...
def build_model_swinbackbone_without_criterion(args):
    ...
    if args.lprop_mode > 0:
        ...
        model = VOS_SwinLPROP(args, backbone, backbone_dims=backbone_dims, hidden_dim=args.hidden_dim,
        ...
        model = VOS_Swin(args, backbone, backbone_dims=backbone_dims, hidden_dim=args.hidden_dim,
                              transformer=transformer, num_frames=args.num_frames, n_class=args.num_classes)

    return model


def build_model_swinbackbone(args):
    # Model
    model = build_model_swinbackbone_without_criterion(args)
    # Losses
    weight_dict = {"loss_mask": args.mask_loss_coef, "loss_dice": args.dice_loss_coef,
     "loss_prior_0": args.prior_loss_0_coef, "loss_prior_1": args.prior_loss_1_coef, 
     "loss_prior_2": args.prior_loss_2_coef, "loss_prior_3": args.prior_loss_3_coef}
