...
from emoformer.utils.misc import interpolate
...
def crop(clip, target, region):
    cropped_image = []
    for image in clip:
        img_crop = F.crop(image, *region)
        cropped_image.append(img_crop)
    target = target.copy()
    i, j, h, w = region
    ...
    if "priors" in target:
        target['priors'] = target['priors'][:, i:i + h, j:j + w]
        fields.append("priors")
        

    if 'flows' in target:
        cropped_flows = []
        for flow in target['flows']:
            flow_crop = F.crop(flow, *region)
            cropped_flows.append(flow_crop)
        target['flows'] = cropped_flows
...
def hflip(clip, target):
    ...
    if "priors" in target:
        target['priors'] = target['priors'].flip(-1)
        
    if 'flows' in target:
        flipped_flows = []
        for flow in target['flows']:
            flipped_flows.append(F.hflip(flow))
        target['flows'] = flipped_flows
...
def resize(clip, target, size, max_size=None):
    ...
    if "priors" in target:
        if target['priors'].shape[0] > 0:
            target['priors'] = interpolate(
                target['priors'][:, None].float(), size, mode="nearest")[:, 0] > 0.5
        else:
            target['priors'] = torch.zeros((target['priors'].shape[0], h, w))
            
    if 'flows' in target:
        rescaled_flows = []
        for flow in target['flows']:
            rescaled_flows.append(F.resize(flow, size))
        target['flows'] = rescaled_flows
